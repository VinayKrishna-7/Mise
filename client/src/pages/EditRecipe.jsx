...
const EditRecipe = () => {
  ...
  useEffect(() => {
    document.title = 'MISE — Edit Recipe';
    const fetchRecipeData = async () => {
      try {
        const res = await getRecipe(id);
        const data = res.data.data;
        if (data.title) {
          document.title = `MISE — Edit ${data.title}`;
        }
        if (data.isSystem) {
          addToast('Foundation recipes cannot be edited.', 'info');
          navigate(`/recipes/${id}`);
          return;
        }
        reset({
          ...
          instructions: data.instructions?.length ? data.instructions : [{ description: '' }],
        });
        setTags(data.tags || []);
        setCurrentImage(data.image || '');
      } catch (err) {
        addToast('Failed to load recipe details for editing', 'error');
        navigate('/recipes');
      } finally {
        setIsLoading(false);
      }
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      addToast(err.message || 'Failed to update recipe', 'error');
    } finally {
      setIsSubmitting(false);
    }
  };

  if (isLoading) return <LoadingSpinner text="Loading recipe details..." />;
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 max-w-6xl">
      ...
      <div className="max-w-2xl mb-8">
        ...
        <p className="text-sm sm:text-base text-slate-600 dark:text-slate-400 mt-2 font-normal leading-relaxed">
          Update ingredients, measurements, instructions, or replace the photo.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-28">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-3">
              ...
              <ul className="text-xs text-slate-600 dark:text-slate-400 space-y-2 list-disc list-inside leading-relaxed">
                <li>Double check that quantities and ingredient names are distinct.</li>
                <li>Make sure instructions remain clear for first-time cooks.</li>
                <li>Save changes when you are ready to update the live recipe.</li>
              </ul>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="flex items-center justify-end gap-3 pt-3">
              <Link to={`/recipes/${id}`} className="btn-secondary py-3 px-6 text-sm font-semibold rounded-xl">
                Cancel
              </Link>
              <button
                type="submit"
